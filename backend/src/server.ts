...
// =========================================================================
app.post('/api/cadastro/responsavel', async (req: Request, res: Response): Promise<any> => {
    ...
    try {
        ...
        const hashedSenhaMaster = await bcrypt.hash(senha_master, 10);
...
app.post('/api/login/responsavel', async (req: Request, res: Response): Promise<any> => {
    ...
    try {
        ...
        const token = jwt.sign({ id: usuario.id_master, role: 'master' }, JWT_SECRET, { expiresIn: '1d' });
...
// =========================================================================
app.post('/api/login/dependente', async (req: Request, res: Response): Promise<any> => {
    const { nickname, senha } = req.body;

    if (!nickname || !senha) return res.status(400).json({ error: "Nickname e Senha são obrigatórios!" });
    ...
    try {
        ...
        const token = jwt.sign({ id: usuario.id_dependente, role: 'dependente' }, JWT_SECRET, { expiresIn: '1d' });
...
// =========================================================================
app.post('/api/album/abrir-pacotinho', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    const userRole = (req as any).userRole;
...
// =========================================================================
app.post('/api/trocas/criar-oferta', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    const userRole = (req as any).userRole;
    
    if (userRole !== 'dependente') return res.status(403).json({ error: "Apenas crianças podem criar ofertas." });

    const { id_figurinha_ofertada, id_figurinha_solicitada } = req.body;
    if (!id_figurinha_ofertada || !id_figurinha_solicitada) return res.status(400).json({ error: "Faltam figurinhas na oferta." });
...
// =========================================================================
app.get('/api/trocas/feira-global', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    try {
...
// =========================================================================
app.post('/api/trocas/aceitar-oferta', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    const userRole = (req as any).userRole;
    
    if (userRole !== 'dependente') return res.status(403).json({ error: "Apenas crianças podem aceitar ofertas." });

    const { id_troca } = req.body;
    if (!id_troca) return res.status(400).json({ error: "ID da oferta é obrigatório." });

    try {
        const oferta = await prisma.autorizacoes_troca.findUnique({ where: { id_troca } });
        if (!oferta || oferta.status_troca !== 'Aberta') return res.status(404).json({ error: "Oferta não encontrada ou já aceita." });
        if (oferta.id_crianca_propos === userId) return res.status(400).json({ error: "Você não pode aceitar sua própria oferta." });
...
// =========================================================================
app.get('/api/trocas/pendentes', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    const userRole = (req as any).userRole;
    
    if (userRole !== 'master') return res.status(403).json({ error: "Apenas pais podem acessar aprovações pendentes." });
...
// =========================================================================
app.post('/api/trocas/aprovar', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    const userRole = (req as any).userRole;
    
    if (userRole !== 'master') return res.status(403).json({ error: "Acesso restrito para pais." });

    const { id_troca, aprovado } = req.body; // aprovado: boolean

    try {
        const troca = await prisma.autorizacoes_troca.findUnique({ where: { id_troca } });
        if (!troca || troca.status_troca !== 'Pendente_Aprovacao') return res.status(404).json({ error: "Troca inválida." });
        ...
        if (!isPai1 && !isPai2) return res.status(403).json({ error: "Este filho não pertence a você." });
        ...
        if (isPai1) updateData.pai_1_aprovou = true;
        if (isPai2) updateData.pai_2_aprovou = true;
...
// =========================================================================
app.post('/api/chat/nova-sala', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    const userRole = (req as any).userRole;
    
    if (userRole !== 'dependente') return res.status(403).json({ error: "Apenas crianças podem criar chats." });

    const { nickname_amigo } = req.body;
    if (!nickname_amigo) return res.status(400).json({ error: "Nickname do amigo é obrigatório." });
    ...
    try {
        ...
        if (!crianca1 || !crianca2) return res.status(404).json({ error: "Amigo não encontrado." });
        if (crianca1.id_dependente === crianca2.id_dependente) return res.status(400).json({ error: "Não pode conversar consigo mesmo." });
...
// =========================================================================
app.post('/api/chat/mensagem', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    const userRole = (req as any).userRole;
    
    if (userRole !== 'dependente') return res.status(403).json({ error: "Apenas crianças enviam mensagens." });

    const { id_sala, conteudo } = req.body;
    if (!id_sala || !conteudo) return res.status(400).json({ error: "Sala e conteúdo são obrigatórios." });
...
// =========================================================================
app.get('/api/chat/sala/:id_sala', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    ...
    try {
        ...
        const canRead = 
            (req as any).userRole === 'dependente' ? (sala.id_crianca_1 === userId || sala.id_crianca_2 === userId) :
            (sala.id_pai_1 === userId || sala.id_pai_2 === userId);
            
        if (!canRead) return res.status(403).json({ error: "Sem permissão para ler este chat." });
...
// =========================================================================
app.get('/api/financeiro/saldo-pai', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
...
app.post('/api/financeiro/depositar', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    if ((req as any).userRole !== 'master') return res.status(403).json({ error: "Acesso restrito para pais." });

    const { valor } = req.body;
    if (!valor || valor <= 0) return res.status(400).json({ error: "Valor de depósito inválido." });
...
// =========================================================================
app.post('/api/financeiro/transferir-mesada', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    if ((req as any).userRole !== 'master') return res.status(403).json({ error: "Acesso restrito para pais." });
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            let carteira = await tx.carteira_digital.findUnique({ where: { id_dependente } });
            if (carteira) {
                await tx.carteira_digital.update({
                    where: { id_dependente },
                    data: { saldo_moedas: { increment: moedasConvertidas } }
                });
            } else {
                await tx.carteira_digital.create({
                    data: { id_dependente, saldo_moedas: moedasConvertidas }
                });
            }
        });
...
// =========================================================================
app.get('/api/financeiro/saldo-crianca', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    if ((req as any).userRole !== 'dependente') return res.status(403).json({ error: "Acesso restrito." });

    try {
        let carteira = await prisma.carteira_digital.findUnique({ where: { id_dependente: userId } });
        if (!carteira) {
            carteira = await prisma.carteira_digital.create({ data: { id_dependente: userId, saldo_moedas: 0 } });
        }
        return res.json({ saldo_moedas: carteira.saldo_moedas });
    } catch (error) {
        return res.status(500).json({ error: "Erro ao buscar saldo." });
    }
});

app.post('/api/financeiro/comprar-pacote-com-moedas', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    if ((req as any).userRole !== 'dependente') return res.status(403).json({ error: "Acesso restrito." });
    ...
    try {
        const carteira = await prisma.carteira_digital.findUnique({ where: { id_dependente: userId } });
        if (!carteira || carteira.saldo_moedas < PRECO_PACOTE) {
            return res.status(400).json({ error: "Saldo de moedas insuficiente. Peça mais mesada!" });
        }

        const catalogo = await prisma.figurinhas_catalogo.findMany();
        if (catalogo.length === 0) return res.status(400).json({ error: "O catálogo de figurinhas está vazio." });
        ...
        await prisma.$transaction(async (tx) => {
            ...
            await tx.carteira_digital.update({
                ...
                data: { saldo_moedas: { decrement: PRECO_PACOTE } }
            });
...
// =========================================================================
app.post('/api/apostas/nova', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    const userId = (req as any).userId;
    if ((req as any).userRole !== 'master') return res.status(403).json({ error: "Apenas pais podem apostar." });

    const { valor_apostado, tipo_aposta, odd } = req.body;
    if (!valor_apostado || valor_apostado <= 0) return res.status(400).json({ error: "Valor de aposta inválido." });
...
app.post('/api/apostas/resolver', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    ...
    try {
        ...
        if (!aposta || aposta.status_aposta !== 'Ativa') return res.status(400).json({ error: "Aposta inválida." });
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (resultado === 'Ganha') {
                const premio = Number(aposta.valor_apostado) * Number(aposta.odd);
                await tx.usuarios_master.update({
                    where: { id_master: aposta.id_master },
